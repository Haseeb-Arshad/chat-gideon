...
## Stage 17 procedures (`packages/memory/src/procedures.ts`)

- `validateManifest`, `SqliteProcedureStore`, `openProcedures` →
  `ScopedProcedures` (`recordEpisode`, `propose`, `review`,
  `recordVerification`, `promote`, `rollback`, `deleteEpisode`, `advise`,
  `adviseShadow`, `reviewForEnvironment`, `inspect`, `recordAdoption`),
  `adviceFromManifest`.
- Server: `procedures-inspect` / `procedures-advise` when a store is given.
