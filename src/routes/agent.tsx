...
function extractSpeakableChunks(value: string, flush = false) {
  const chunks: string[] = []
  let remainder = value
  ...
  while ((match = sentenceEnd.exec(remainder))) {
    ...
    const chunk = remainder.slice(consumed, end).trim()
  ...
  while (!flush && remainder.length > 180) {
    const window = remainder.slice(0, 180)
    const punctuation = Math.max(window.lastIndexOf(','), window.lastIndexOf(';'), window.lastIndexOf(':'))
    const whitespace = window.lastIndexOf(' ')
    const splitAt = punctuation > 96 ? punctuation + 1 : whitespace > 96 ? whitespace : 180
...
function HomePage() {
  ...
  async function requestVoiceChunk(text: string, signal: AbortSignal) {
  ...
  function playVoiceChunk(
    ...
    return new Promise<boolean>((resolve) => {
      ...
      const showWords = (count: number) => {
        const nextCount = Math.max(1, Math.min(words.length, count))
        if (nextCount === visibleWords) return
        visibleWords = nextCount
        setAssistantCaption([spokenPrefix, words.slice(0, nextCount).join(' ')]
          .filter(Boolean)
          .join(' '))
      }
      ...
      const followAudio = () => {
        ...
        if (duration) {
          ...
          showWords(Math.ceil(progress * words.length))
        }
      ...
      const finish = (played: boolean) => {
        ...
        if (played) {
          setAssistantCaption([spokenPrefix, text.trim()].filter(Boolean).join(' '))
        }
        releaseAudio()
        resolve(played)
      }
  ...
  async function sendMessage(rawText: string) {
    ...
    const enqueueVoice = (chunk: string) => {
      ...
      if (!cleanChunk || controller.signal.aborted || voiceModeRef.current !== 'active') return
      queuedVoice = true
      const audioPromise = generationTail.then(async () => {
        try {
          return { blob: await requestVoiceChunk(cleanChunk, controller.signal), error: null }
        } catch (error) {
          return { blob: null, error }
        }
      })
      generationTail = audioPromise.then(() => undefined)
      speechTail = speechTail.then(async () => {
        if (controller.signal.aborted || voiceModeRef.current !== 'active') return
        ...
        if (!result.blob || controller.signal.aborted || voiceModeRef.current !== 'active') return
        const played = await playVoiceChunk(cleanChunk, result.blob, spokenPrefix, controller.signal)
        if (played) spokenPrefix = [spokenPrefix, cleanChunk].filter(Boolean).join(' ')
        else if (!controller.signal.aborted && voiceModeRef.current === 'active') voiceFailed = true
      })
    }
    ...
    try {
      ...
      const completeText = await consumeOpenRouterStream(response, (delta) => {
        if (voiceModeRef.current === 'active') {
          ...
          extracted.chunks.forEach(enqueueVoice)
        } else {
      ...
      if (voiceModeRef.current === 'active') {
        const finalChunks = extractSpeakableChunks(speechBuffer, true)
        finalChunks.chunks.forEach(enqueueVoice)
      }
      ...
      if (!queuedVoice || voiceFailed || voiceModeRef.current !== 'active') {
        setAssistantCaption(completeText)
      }

      setPhase(voiceModeRef.current === 'paused' ? 'paused' : 'idle')
      if (voiceModeRef.current === 'active' && !controller.signal.aborted) {
        setTimeout(() => startListeningRef.current(false), 160)
      }
    } catch (error) {
      if ((error as Error).name !== 'AbortError') {
  ...
  const statusCopy = voiceMode === 'paused' ? 'Quiet pause' : {
      ...
      <section className="agent-presence" aria-label="GIDEON voice presence">
        ...
        <div className="live-captions" aria-live="polite" aria-atomic="false">
          ...
          <p className={`assistant-caption ${phase === 'thinking' && !assistantCaption ? 'is-thinking' : ''}`}>
            {assistantCaption ? assistantCaption.split(/\s+/).map((word, index) => (
              <span className="caption-word" key={`${captionTurn}-${index}`}>{word}</span>
