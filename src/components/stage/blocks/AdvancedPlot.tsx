...
export function AdvancedPlot({ block: b, width, height, front, focus, onFocus, summary }: Props) {
  ...
  if (['dot', 'dumbbell', 'bullet', 'box'].includes(form)) {
    ...
    const x1 = 26, x2 = width - 26
    drawing = <><text x={x1} y={14} className="chart-x">{short(series[0].label)}</text><text x={x2} y={14} textAnchor="end" className="chart-x">{short(series[1].label)}</text>{x.map((_, i) => mark(i, <><line x1={x1} x2={x2} y1={sy(series[0].values[i]!)} y2={sy(series[1].values[i]!)} stroke={color(i)} strokeWidth={focus === i ? 4 : 2} strokeDasharray={i % 2 ? '6 3' : undefined} /><circle cx={x1} cy={sy(series[0].values[i]!)} r={4} fill={color(i)} /><rect x={x2 - 4} y={sy(series[1].values[i]!) - 4} width={8} height={8} fill={color(i)} /></>))}</>
  ...
  return <div className="advanced-chart">
    ...
    {form === 'editorial' ? <nav className="advanced-story" aria-label="Source story"><button type="button" disabled={(focus ?? 0) <= 0} onClick={() => onFocus(Math.max(0, (focus ?? 0) - 1))}>Previous</button><span>{(focus ?? 0) + 1} / {x.length}</span><button type="button" disabled={(focus ?? 0) >= x.length - 1} onClick={() => onFocus(Math.min(x.length - 1, (focus ?? 0) + 1))}>Next</button></nav> : null}
